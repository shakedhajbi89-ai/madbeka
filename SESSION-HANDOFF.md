...
## 💻 פקודות טרמינל שיהיה צריך

כל הפקודות מתבצעות בתוך תיקיית הפרויקט (`C:\Users\shake\claude code project\Madbeka`). פתיחת טרמינל בתיקייה: קליק ימני בתיקייה → "Open in Terminal".

**להריץ את האתר מקומית (לבדיקה):**
```
npm run dev
```
ואז פותחים דפדפן ב-`http://localhost:3000`. לעצירה: `Ctrl+C`.

**גיבוי שבועי (אחרי שינויים):**
```
git add .
